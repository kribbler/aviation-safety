		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'radio':
					...
					break;
				case 'checkbox':
					echo '<input type="checkbox" name="', $field['id'], '" id="', $field['id'], '"', $meta=='on' || (!metadata_exists('post', $post->ID, $field['id']) && $field['std']=='on') ? ' checked="checked"' : '', ' />'; // if meta==='' then user has unselected it
...
function cmb_scripts( $hook ) {
	...
	if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
		...
		wp_register_script( 'cmb-timepicker', $codeNegar_ctt->url . 'js/timePicker.js' );
		wp_register_script( 'cmb-scripts', $codeNegar_ctt->url . 'js/cmb.js', $cmb_script_array, '0.9.1' );
		...
		wp_register_style( 'cmb-styles', $codeNegar_ctt->url . 'css/metabox.css', $cmb_style_array );
...
function cmb_oembed_ajax_results() {
	...
	if ( empty( $oembed_string ) ) {
		$return = '<p class="ui-state-error-text">'. __( 'Please Try Again', $codeNegar_ctt->text_domain ) .'</p>';
		$found = 'not found';
	} else {
		...
		if ( $check_embed && $check_embed != $fallback ) {
			// Embed data
			$return = '<div class="embed_status">'. $check_embed .'<a href="#" class="cmb_remove_file_button" rel="'. $_REQUEST['field_id'] .'">'. __( 'Remove Embed', $codeNegar_ctt->text_domain ) .'</a></div>';
			// set our response id
			$found = 'found';

		} else {
			// error info when no oEmbeds were found
			$return = '<p class="ui-state-error-text">'.sprintf( __( 'No oEmbed Results Found for %s. View more info at', $codeNegar_ctt->text_domain ), $fallback ) .' <a href="http://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>';
			// set our response id
			$found = 'not found';
		}
	}
